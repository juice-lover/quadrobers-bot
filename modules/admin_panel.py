...
def send_msg_all_users_function(bot, message):
    ...
    # Расчет примерного времени завершения рассылки
    estimated_time = total_users * delay
    end_time = datetime.now() + timedelta(seconds=estimated_time)
    ...
    admin_message = (
        ...
        f"Примерное время завершения: {end_time.strftime('%H:%M:%S')}\n"
    ...
    final_message = (
        ...
        f"Ошибок: {failed}\n"
        f"Общее время рассылки: {estimated_time // 60} мин {estimated_time % 60} сек"
    )
